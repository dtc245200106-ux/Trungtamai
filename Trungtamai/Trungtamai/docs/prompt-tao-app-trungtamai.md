...
## 11. Nguyên tắc triển khai bổ sung

### 11.1. Nguyên tắc phản hồi của AI

- Chỉ sử dụng dữ liệu thực tế được cung cấp từ hệ thống.
- Không tự tạo tên, mã học viên, mã giáo viên, tên lớp hoặc số liệu không tồn tại.
- Khi không có dữ liệu, phải nói rõ "Hiện chưa có dữ liệu phù hợp".
- Khi người dùng hỏi số lượng, phải trả về con số cụ thể và nêu phạm vi thống kê.
- Khi người dùng hỏi danh sách, phải liệt kê các bản ghi thực tế phù hợp với quyền truy cập.
- Khi so sánh, phải dựa trên các trường dữ liệu đã được cung cấp.
- Số tiền phải có đơn vị `đ` hoặc `VND`.
- Ngày phải hiển thị đầy đủ theo dạng `dd/MM/yyyy`.
- Thời gian phải hiển thị theo dạng 24 giờ `HH:mm`.
- Câu trả lời phải bằng tiếng Việt, dễ đọc và kết thúc bằng dấu chấm.
- Không tư vấn vượt quá dữ liệu hoặc chức năng mà hệ thống đang hỗ trợ.

### 11.2. Nguyên tắc bảo mật

- Không commit Gemini API key, mật khẩu database hoặc thông tin bí mật vào Git.
- Có thể đọc Gemini API key từ `Gemini:ApiKey` hoặc biến môi trường `GEMINI_API_KEY`.
- Không gửi dữ liệu ngoài phạm vi vai trò vào data payload cho Gemini.
- Kiểm tra quyền ở cả controller, direct handler và service truy vấn dữ liệu.
- Học viên không được xem dữ liệu của học viên khác.
- Giáo viên không được xem lớp hoặc điểm của lớp không được phân công.
- Tư vấn viên không được xem điểm số hoặc học phí cá nhân ngoài phạm vi được phép.
- Logout phải xóa toàn bộ session.
- Session cookie phải đặt `HttpOnly` và thời gian hết hạn là 8 giờ.

### 11.3. Nguyên tắc dữ liệu và giao diện

- Sử dụng Entity Framework Core và migration để quản lý schema database.
- Các truy vấn danh sách phải xử lý trường hợp dữ liệu liên quan chưa tồn tại.
- Trang kết quả học tập phải bắt đầu từ bảng `HocVien` và LEFT JOIN sang `DiemSo` để hiển thị cả học viên chưa có điểm.
- Giao diện, nhãn, thông báo lỗi và câu trả lời chatbot phải bằng tiếng Việt.
- Giao diện phải responsive trên desktop, tablet và mobile.
- Giữ logic nghiệp vụ trong controller/service, không đặt truy vấn phức tạp trong Razor View.
- Ưu tiên các direct handler trước khi gọi Gemini để giảm chi phí và thời gian phản hồi.

## 12. Tiêu chí nghiệm thu

Ứng dụng chỉ được xem là hoàn thành khi đáp ứng các tiêu chí sau:

1. Project build thành công bằng .NET 10.
2. Có thể restore package bằng `dotnet restore backend/Trungtamai.csproj`.
3. Có thể khởi động bằng `dotnet run --project backend/Trungtamai.csproj`.
4. Database được tạo hoặc cập nhật bằng Entity Framework migration.
5. Người dùng có thể đăng nhập và được chuyển đến đúng trang theo vai trò.
6. Người dùng không thể truy cập trang ngoài quyền hạn của mình.
7. Các chức năng quản lý chính sử dụng dữ liệu từ SQL Server hoặc LocalDB.
8. Chatbot trả lời được các câu hỏi direct handler mà không cần gọi Gemini.
9. Chatbot chỉ gửi data payload phù hợp với vai trò người dùng.
10. Lịch sử hội thoại được lưu vào bảng `ChatMessages`.
11. API key không xuất hiện trong mã nguồn hoặc tài liệu công khai.
12. Tài liệu README, phân quyền, luồng chatbot và kịch bản kiểm thử được cập nhật theo mã nguồn thực tế.

## 13. Phạm vi chưa triển khai

Không tự ý thêm các chức năng sau nếu không có yêu cầu mới:

- Thanh toán trực tuyến qua cổng thanh toán.
- Dự báo nguy cơ nghỉ học bằng AI hoặc machine learning.
- Chấm điểm bài làm tự động bằng AI.
- Kiểm thử chịu tải quy mô lớn và cam kết số lượng người dùng đồng thời.
- Các chức năng nghiệp vụ không được mô tả trong prompt này.

## 14. Cách sử dụng prompt

Khi triển khai hoặc mở rộng dự án, hãy đọc toàn bộ prompt này trước khi sửa code. Mọi thay đổi phải:

- Giữ tương thích với cấu trúc thư mục hiện tại.
- Ưu tiên tái sử dụng model, service và pattern đã có.
- Không làm mất dữ liệu hoặc thay đổi migration cũ ngoài yêu cầu.
- Cập nhật tài liệu liên quan khi thay đổi hành vi hệ thống.
- Kiểm tra build hoặc test phù hợp sau mỗi thay đổi.
